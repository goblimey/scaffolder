...
implemented using RESTful requests.  (Those buzzwords are explained in more detail below.)
...
The web server generated by the scaffolder can be configured to connect to this database as this user.
...
As we will see later, git uses a two-stage commit.
Once you have created some files, you first commit them to your local repository.
...
When you are happy with your project, you can "push" the files to the github.com server, 
at which point they become public and other people can get at them.
...
The web application server provides controlled access to data in the database. Each response is manufactured to order, based on the data. In a production system, the user can't access the database directly, only via the web server.
...
The web server sends back an HTTP response.   This could contain an Excel spreadsheet, an Acrobat document, a sound file, a video or pretty much anything that can be stored on a computer.
...
The scaffolder generates a web server including complete source code, so you can see how it works.  Once you understand that, you can use it as a starting point to create your own server.

To understand the existing design, look at the source code and work through the logic that handles a request, starting with the main program animals.go.
