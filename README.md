...
For the Impatient
============
To get the dependencies and install the scaffolder
...
Before you start, you need to install the Go tools.  If you've already done that, skip to the next section.
...
You must start with a github project and then clone a copy of it on your computer.
So the next thing to do is create a github project.
...
you now need to create some subdirectories
in your goprojects directory.
...
Now you can clone your github project.  
...
That creates a directory "animals" containing a copy of any files you created on the github site
and also some magic hidden files that make it a local git repository.
...
Using a command window (LInux or Windows), run these commands:
...
These commands download ready-made Go projects onto your computer, including the scaffolder.
Your GOPATH variable says where they will be stored.
...
You added that directory to your path earlier, which makes It's easier to run programs in it.
Just type the name of the executable file.
...
The database takes care of the ID field.  When you create a record you don't have to set it.
...
When you run the scaffolder, by default it looks for a file "scaffold.json" in the current directory
defining the database - something like the example above.  You can specify a different file.

By default the scaffolder generates a Go project in the current directory, which should be your github project directory (in the example, goprojects/animals).
Alternatively you can run it while in another directory and tell it where to find the project directory.
...
Copy the file animals.scaffold.json to your project directory, rename it scaffold.json and edit it to set your sourceBase ("github.com/alunsmithie/animals" or whatever).

In your command window, cd to your project directory and run the scaffolder

    $ scaffolder

That creates the source code of your web server and some scripts.  install.sh  builds and installs the server on Linux:
...
If the tests work, you can start the web server:
...
The git repository keeps track of changes to files.  It uses a two-stage commit.
Once you have created some files, you first commit them to the local repository,
which in this case is hidden in your project directory.
...
at which point the files become public and other people can get at them.
...
So just committing changes your local repository is useful even if you don't plan to publish the project on Github. Unfortunately, that requires you to open a Github account and create an empty project, just so you can clone it.  If that irks you, you can install your own git server.

To update the public version of the project on the Github, push the current state of the repository:
...
The scaffolder generates a web server including complete source code, so you can see how it works.  You can use this as a starting point to create your own server.

The easiest change is to tweak the views.  For example, the beginning and the end of the HTML of most pages is controlled by the base template views/_base.ghtml.  To add a standard banner to the top of those pages, edit that file.  The only other pages are the home page and the static error page in views/html.  You will need to edit those in the same way.  There also a stylesheet in views/stylesheet. You can add your own styling controls there.

More complex enhancements will require changes to the Go source code, but before you can do that,
you need to understand the existing design,.  Read the source code and work through the logic that handles a request, starting with the main program animals.go.

When the web server starts, the main method specifies which requests the server will handle.  It registers each request that it will handle and arranges that when any of them arrive, the marshal method is called to handle it, The marshal method figures out which controller to use and which method to call.

The main method starts the server, which runs until it's stopped.  It waits for HTTP requests and processes each one, producing an HTTP response, in this case always an HTML web page.
...
If you want to write your own repository, follow a similar line of attack.  The repositories are supplied to the controllers using the service object, so you need to tweak that to supply your version.  It's set up at the start of the main module's marshal method.

Test Driven Development
==================

The server source code is set up the way it is, with objects defined as interfaces, to support Test Driven Development, which basically means: write a bit of code, write tests for it, test it thoroughly, get it working completely, then continue to the next bit.  I urge you very strongly to follow that approach.

The scaffolder generates unit tests and integration tests.  A unit test test a module of source code in isolation, without involving any of the other modules.  When the module under test depends on another module, it's supplied with a fake version that satisfies the same interface.
...
The test script runs through a specific set of directories and runs all the tests in them. 
As you add your own modules, write tests and keep the script up to date as you add directories to the source tree.

You can, of course, test everything by hand.  The problem comes when you change anything.  You need to run all the tests again to make sure that everything still works.  It's very easy to fix one thing and break another.  Recording a set of automated tests that run quickly and do a thorough test of the basic logic ensures a more consistent approach.

The unit tests that are generated make extensive use of a technique called mocking.  Mocking uses clever objects (mocks) that satisfy an interface and can be made to perform to order during a test.  I use [pegomock](https://github.com/petergtz/pegomock) to produce mocks.  The test scripts (test.sh for Linux and test.bat for Windows) generate the mocks before running the tests.

Once you have created your mock, you can tell it what to do. For example the program controller_test.sh in the cat controller module contains tests for the cat controller. 
The first one tests that the controller produces the right response when everything works OK.
It creates mock cats repository and a list of cats called expectedCatList.  Then it does this:
